...
if [[ $(/usr/sbin/sysctl -n machdep.cpu.brand_string | /usr/bin/grep "Apple") ]]; then
    ...
    /usr/bin/curl -sL -o "/tmp/${appName}/${appName}.dmg" "https://desktop.docker.com/mac/main/arm64/Docker.dmg"
else
    # Intel version of the application.
    writeToLog "[INFO] Computer has Intel processor, downloading ${appName}..."
    /usr/bin/curl -sL -o "/tmp/${appName}/${appName}.dmg" "https://desktop.docker.com/mac/main/amd64/Docker.dmg"
fi
...
# Using osascript cleanly closes Docker, as killall breaks the app.
if [ -d "/Applications/${appName}.app" ]; then
