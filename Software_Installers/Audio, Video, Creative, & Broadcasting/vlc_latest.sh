...
###################################
appName="VLC"
appVers=$(/usr/bin/curl -s http://www.videolan.org/vlc/download-macosx.html | /usr/bin/grep "Universal Binary" | /usr/bin/head -1 | /usr/bin/sed 's/.*vlc-//g; s/-universal.dmg//g; s/">VLC//g' | /usr/bin/awk '{print $1}')
...
logFile="/Library/Logs/Install/VLC.log"
...
/usr/bin/curl -sL -o "/tmp/${appName}/${appName}.dmg" "https://get.videolan.org/vlc/${appVers}/macosx/vlc-${appVers}-universal.dmg"
/usr/bin/curl -sL -o "/tmp/${appName}/${appName}.dmg.md5" "https://get.videolan.org/vlc/${appVers}/macosx/vlc-${appVers}-universal.dmg.md5"

# Calculate the MD5 for the file.
writeToLog "[INFO] Calculating MD5 for ${appName}..."
appSHA=$(/bin/cat /tmp/${appName}/${appName}.dmg.md5 | awk '{print $1}')
fileSHA=$(openssl md5 /tmp/${appName}/${appName}.dmg | awk '{print $2}')
