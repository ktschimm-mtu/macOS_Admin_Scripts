...
logFile="/Library/Logs/Install/Firefox ESR.log"
...
/usr/bin/curl -sL -o "/tmp/${appName}/${appName}.dmg" "https://download.mozilla.org/?product=firefox-esr-latest-ssl&os=osx&lang=en-US"
...
/bin/ps aux | /usr/bin/grep -v grep | /usr/bin/grep "${appName}".app
    if [ "$?" -eq 0 ]; then
        ...
        /usr/bin/killall $(echo "${appName}" | awk '{print tolower($0)}')
