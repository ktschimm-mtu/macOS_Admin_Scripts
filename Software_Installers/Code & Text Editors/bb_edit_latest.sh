...
###################################
appName="BBEdit"
appVers=$(curl -s https://www.barebones.com/support/bbedit/updates.html | grep "<li>" | grep "#new" | sed 's/<[^>]*>//g' | awk '{print $NF}')
...
logFile="/Library/Logs/Install/BBEdit.log"
...
/usr/bin/curl -sL -o "/tmp/${appName}/${appName}.dmg" "https://s3.amazonaws.com/BBSW-download/${appName}_${appVers}.dmg"
###################################
## Installation
###################################
